...
class VirtualQueueTypeSelection extends Component
{
    public $locationId;
    public $teamId;
    public $settings;
    ...
    {
        $this->locationId = base64_decode($location_id,true) ?? Session::get('selectedLocation');
        ...
        if (!$this->settings->isVirtualQueueEnabled()) {
            ...
            return redirect()->route('queue', ['location_id' => $this->locationId]);
        }
    }
    ...
    public function createVirtualQueue()
    {
        ...
        if ($this->selectedType === 'ai_agent') {
            return redirect()->route('ai-agent-call-staff', ['virtualQueueId' => base64_encode($virtualQueue->id)]);
        } else {
            return redirect()->route('human-agent-waiting-staff', ['virtualQueueId' => base64_encode($virtualQueue->id)]);
        }
    }
    ...
    {
        return view('livewire.virtual-queue-type-selection');
    }
}
