...
class QueueOverview extends Component
{
...
public function loadCategories()
{
    ...
    $this->categories = $query->select([
        ...
        ->groupBy('category_id')
        ->orderBy('category_id')
        ->with('category:id,name,category_locations') // Efficient eager loading
        ->get();
...
public function getTotals()
{
    ...
    {
        $locations = Location::where('team_id',$this->teamId)->pluck('location_name', 'id')->toArray();
        return view('livewire.branches.queue-overview',compact('locations'));
    }
}
