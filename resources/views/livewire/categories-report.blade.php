<div>
    ...
    <main>
        <div class="mx-auto p-4 md:p-4">
            ...
            <h1 class="text-xl font-semibold dark:text-white/90 mb-4">{{ __('report.Services Report') }}</h1>
            ...
            <div class="flex flex-col space-y-2 md:space-y-0 md:flex-row md:space-x-2 level-selector">
                ...
                <!-- Date Range Filters -->
                <div class="flex space-x-2 md:mt-0">
            ...
            <div class="p-4 md:p-4 rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow">
                ...
                <table class="table-auto w-full  ti-custom-table ti-custom-table-hover">
                    <thead class="">
                         <!-- Main Headings Row -->
                        <tr>
                            <th colspan="10" class="px-4 py-2">{{ __('report.Customers transactions') }}</th>
                            <th colspan="3" class="px-4 py-2">{{ __('report.Transaction time') }}</th>
                            <th colspan="6" class="px-4 py-2">{{ __('report.Waiting time') }}</th>
                        </tr>

                        <!-- Sub-Headings Row -->
                        <tr>
                            <th class="px-4 py-2"><?php echo __('report.Queue'); ?></th>
					...
					?>
                        ...
                        @endforeach
                        <tr>
                            <td class=""><strong><?php echo __('Total'); ?></strong></td>
                            <td class="">
			...
			<div class="text-sm text-gray-600 space-y-1 mt-3">
			    <div><strong>{{ __('report.SL') }}:</strong> {{ __('report.Service Limit') }}</div>
			    <div><strong>{{ __('report.> SL') }}:</strong> {{ __('report.Served within service time') }}</div>
			    <div><strong>{{ __('report.< SL') }}:</strong> {{ __('report.Served exceeded service time') }}</div>
			</div>
			                </div>
...
document.addEventListener("DOMContentLoaded", function() {
    $(document).ready(function() {
        ...
        const select = $('#status').select2({
            placeholder: "{{ __('report.status') }}",  // Optional placeholder
            width: '100%'
        });
