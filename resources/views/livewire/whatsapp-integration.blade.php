<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
    <form wire:submit.prevent="save" class="space-y-4">
        <h2 class="text-xl font-semibold text-gray-700">{{ __('setting.Whatsapp API Integration') }}</h2>
            ...
            <div>
                ...
                <div class="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-4 mt-2 text-sm text-yellow-700 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-700">
                    ...
                    <ul class="list-disc list-inside mt-2">
                        ...
                        <li><code>route</code>: {{ __('setting.Route type (e.g., "4" for transactional SMS).') }}</li>
                    ...
                    <p class="mt-2">
                        <em>{{ __('setting.Note:') }}</em> {{ __('setting.WhatsApp messages must use pre-approved templates. You can create and manage templates in your') }} <a href="https://msg91.com/help/how-to-create-a-template-for-whatsapp" class="text-blue-600 underline" target="_blank">{{ __('setting.Msg91 dashboard') }}</a>.
                    </p>
                </div>
                @endif
        ...
        <div class="col-span-8 flex gap-2 items-center mt-4">
            ...
            <input 
                type="text" 
                wire:model.defer="sms_api_url" 
                placeholder="{{ __('setting.Enter API URL') }}" 
                class="w-full dark:bg-dark-900 h-11 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
            >
            @error('sms_api_url') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        ...
        <button type="button" wire:click="addParameter" class="px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
            + {{ __('setting.Add Parameter') }}
        </button>
