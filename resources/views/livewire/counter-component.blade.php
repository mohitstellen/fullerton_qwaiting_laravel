<div class="p-4">
    ...
    <div class="flex justify-between">
        <h2 class="text-xl font-semibold dark:text-white/90 mb-4">{{ __('text.Counters') }}</h2>
    ...
    <div class="mb-4 flex gap-3 justify-between mb-4 flex-wrap">
            ...
            @can('Counter Add')
         <a href="{{ route('tenant.add-counter') }}" class="primary-btn py-2 px-3 font-medium text-white transition-colors rounded-md bg-brand-500 hover:bg-brand-600 flex gap-x-2"><i class="ri-add-circle-line"></i> {{ __('text.Add Counter') }}</a>
        @endcan
</div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-auto">
                ...
                <thead>
                    ...
                    <th class="px-5 py-3 sm:px-6" style="text-align: justify;">
                        ...
                        @if(Auth::user()->hasAnyPermission(['Counter Edit','Counter Delete']))
                        <th class="px-5 py-3 sm:px-6">
                                    {{ __('text.Actions') }}
                        </th>
                        @endif
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <td class="px-5 py-4 sm:px-6">
                        <input type="checkbox" class="select-checkbox cursor-pointer" value="{{ $counter->id }}">
                    </td>
                        <td class="px-5 py-4 sm:px-6">
                                {{ $counter->name }}
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                                 {{ $counter->show_checkbox ? 'Active' : 'Inactive' }}
                        </td>
                        @if(Auth::user()->hasAnyPermission(['Counter Edit','Counter Delete']))
                        <td class="px-5 py-4 sm:px-6">
                            <div class="">
                                <div x-data="{openDropDown: false}" class="relative">
                                    ...
                                    <div x-show="openDropDown" @click.outside="openDropDown = false" class="dropdown-menu shadow-theme-lg dark:bg-gray-dark absolute top-full left-0 z-40 w-40 space-y-1 rounded-2xl border border-gray-200 bg-white p-2 dark:border-gray-800" style="display: none;">
                                       @can('Counter Edit')
                                    <a href="{{ route('tenant.edit-counter', $counter->id) }}" class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                            <button>
                                                 {{ __('text.Edit') }}
                                            </button>
                                        </a>
                                        @endcan
                                        @can('Counter Delete')
                                        <button class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300" wire:click="delete({{ $counter->id }})">
                                            {{ __('text.Delete') }}
                                        </button>
                                        @endcan
                                    </div>
