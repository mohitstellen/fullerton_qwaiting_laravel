<div class="p-4">
    <div class="flex justify-between mb-4">
        <h2 class="text-xl font-semibold dark:text-white/90">{{ __('text.Dynamic Reports') }}</h2>
    ...
    <div class="mb-4 flex gap-3 justify-between ">
            ...
            <div  class="relative w-full lg:w-[300px]">
                ...
                <a href="{{ route('tenant.create-dynamic-report') }}" class="px-3 py-2 font-medium text-white transition-colors rounded-md bg-brand-500 hover:bg-brand-600 flex gap-x-2"><i class="ri-add-circle-line"></i> {{ __('text.Add') }}</a>
        ...
        <div class="max-w-full overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead>
                    ...
                    <tr>
                            <th class="px-5 py-3 sm:px-6">Name</th>
                            <th class="px-5 py-3 sm:px-6">Report Link</th>
                            <th class="px-5 py-3 sm:px-6">Fields</th>
                ...
                @foreach ($reports as $report)
                    <tr>
                        ...
                        <td class="p-3"> <a href="{{ route('tenant.dynamic-report', base64_encode($report->id)) }}" class="text-blue-600 hover:underline">
        {{ __('text.report link') }}
    </a></td>
                        <td class="p-3"><ul class="list-disc pl-5">
                                @foreach($report->report_fields ?? [] as $field)
                                    <li>{{ $field }}</li>
                                @endforeach
                            </ul>
                        </td>
                        ...
                        <td class="p-3">
                            ...
                            <div>
                                <div x-data="{openDropDown: false}" class="relative w-[45px]">
                                    ...
                                    <a href="{{ route('tenant.edit-dynamic-report', $report->id) }}" class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                           {{ __('text.Edit') }}
                                    </a>
                                        
                                        <button class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300" wire:click="deleteconfirmation({{ $report->id }})">
                                            {{ __('text.Delete') }}
                                        </button>
