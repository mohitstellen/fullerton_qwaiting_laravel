<div class="p-4">
    ...
    <div class="mb-4 flex gap-3 justify-between ">
            ...
            <div  class="relative w-full lg:w-[300px]">
             ...
             @can('Role Add')
                <a href="{{ route('tenant.create-role') }}" class="px-3 py-2 font-medium text-white transition-colors rounded-md bg-brand-500 hover:bg-brand-600 flex gap-x-2"><i class="ri-add-circle-line"></i> {{ __('text.Add') }}</a>
            @endcan
                        ...
                        <td class="p-3">
                            ...
                            <div>
                                <div x-data="{openDropDown: false}" class="relative w-[45px]">
                                    ...
                                    @can('Role Edit')
                                    <a href="{{ route('tenant.edit-role', $role->id) }}" class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                           {{ __('text.Edit') }}
                                        </a>
                                        @endcan
                                        @can('Role Delete')
                                        <button class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300" wire:click="deleteconfirmation({{ $role->id }})">
                                            {{ __('text.Delete') }}
                                        </button>
                                        @endcan
                                    </div>
