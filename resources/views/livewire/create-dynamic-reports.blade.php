<div class="p-4">
    <h2 class="text-xl font-semibold dark:text-white/90 mb-4">{{ __('text.Create Dynamic Report') }}</h2>
    <div class="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-md p-4">
        ...
        <form wire:submit.prevent="saveReport">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2  p-3">
                    ...
                    @foreach ($availableFields as $key => $field)
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" value="{{ $key }}" wire:model="report_fields"
            ...
            <button type="submit"
                class="px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">{{ __('text.Save') }}</button>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                Swal.fire({
                    ...
                    // confirmButtonText: 'OK'
                        ...
                        window.location.href = '/dynamic-reports-list'; // Refresh the page when OK is clicked
