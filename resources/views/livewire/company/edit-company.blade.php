<div class="p-4">
    <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <form wire:submit.prevent="update" class="space-y-6">
                {{-- Company Information and Contact Details Grid --}}
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[0.9fr_1.3fr]">
                    {{-- Left Column: Company Information + Appointment Type Validity --}}
                    <div class="space-y-6 min-w-0">
                        <div>
                            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Company Information</h3>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs">
                                        <thead class="bg-gray-50 dark:bg-gray-800">
                                            <tr>
                                                <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                                                    S.No
                                                </th>
                                                <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                                                    Appointment Type
                                                ...
                                                <th scope="col" class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                                                    Actions
                                                </th>
                                            </tr>
                                        </thead>
                                                ...
                                                <td class="px-3 py-2 whitespace-nowrap text-right text-xs font-medium">
                                                    <div class="flex items-center justify-end gap-1">
                                                        <button type="button"
                                                            ...
                                                            class="inline-flex items-center justify-center rounded border border-gray-300 p-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800"
                                                            title="Edit">
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            class="inline-flex items-center justify-center rounded border border-red-200 p-1.5 text-xs font-medium text-red-600 hover:bg-red-50 dark:border-red-900/40 dark:text-red-300 dark:hover:bg-red-900/20"
                                                            title="Delete"
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                            ...
                                            @empty
                                            <tr>
                                                <td colspan="6" class="px-3 py-2 text-center text-xs text-gray-500 dark:text-gray-400">
        ...
        <div id="company-packages-modal" class="p-5 modal-close-btn inset-0 m-auto w-full max-w-2xl bg-white-400/50 bg-white rounded-xl relative dark:bg-gray-800 dark:border-gray-700 shadow-xl">
            ...
            <div class="overflow-y-auto" style="height: calc(100% - 110px);">
                <form wire:submit.prevent="saveMapping" class="space-y-5">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="relative" x-data="{ open: false }" x-on:click.outside="open = false">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Appointment types *</label>
                            <div class="relative mt-1">
                                <input type="text"
                                    wire:model.live="packageAppointmentTypeSearch"
                                    x-on:click="open = true"
                                    x-on:focus="open = true"
                                    placeholder="Type to search and select appointment types..."
                                    class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 pr-10 text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                                    autocomplete="off">
                                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                    <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Selected appointment types display -->
                            @if(!empty($mappingForm['appointment_type_ids']))
                            <div class="mt-2 flex flex-wrap gap-2">
                                @foreach($appointmentTypes as $type)
                                @if(in_array((string)$type['id'], $mappingForm['appointment_type_ids'] ?? []))
                                <span class="inline-flex items-center gap-1 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-200">
                                    {{ $type['name'] }}
                                    <button type="button" wire:click="selectPackageAppointmentType({{ $type['id'] }}, '{{ $type['name'] }}')"
                            ...
                            <div x-show="open"
                                ...
                                <label class="flex items-center gap-2 px-4 py-2 hover:bg-blue-50 dark:hover:bg-gray-700 cursor-pointer">
                                    <input type="checkbox"
                                    ...
                                    <span class="text-sm text-gray-900 dark:text-gray-100">{{ $type['name'] }}</span>
                                </label>
                                @empty
                                <div class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            @error('mappingForm.appointment_type_ids')
                            <span class="text-sm text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
